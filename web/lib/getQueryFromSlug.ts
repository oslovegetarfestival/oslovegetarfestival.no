...
export const getQueryFromSlug = (slugArray = []) => {
  ...
  const sanityQuery = {
    ...
    eventMain:
      '*[_type == "page" && slug.current == $slug][0] {..., "items": *[_type == "event" && startDateTime > "2025-04-01"] | order(startDateTime) {..., location->{title}}, "highlightedEvents": *[_id == "eventHighlight"][0].highlightedEvents[]->{..., location->{title}}}',
    event:
